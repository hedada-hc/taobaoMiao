...
<script type="text/javascript">
	...
	export default{
		...
		created(){
			this.allCookies = JSON.parse(this.hezone.localQuery("all_cookie"));
			this.queryOrder();
		},
		methods:{
			queryOrder(){
				var sign = this.hezone.deSign(this.allCookies[0].cookie, `{"spm":"a2141.7756461.2.6","page":1,"tabCode":"all","appVersion":"1.0","appName":"tborder"}`);
				...
				request.get({url:url,headers:{"Cookie":this.allCookies[0].cookie}},(error, response, body)=>{
					body = body.replace(/callback\(/,"");
					body = body.replace(/\}\)/,"}");
					this.OrderList = JSON.parse(body);
					if(this.OrderList.ret[0] == "FAIL_SYS_TOKEN_EXOIRED::令牌过期" && !this.todo){
						this.allCookies[0].cookie = this.hezone.replaceCookie(this.allCookies[0].cookie,response.headers['set-cookie']);
						this.todo = true;
